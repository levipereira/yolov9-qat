...
def create_train_dataloader(train_path, imgsz, batch_size, single_cls, stride, hyp_path, mask_ratio, overlap):
...
def create_val_dataloader(test_path, imgsz, batch_size, single_cls, stride, keep_images, mask_ratio,overlap ):
    loader = create_dataloader(
        test_path, 
        imgsz=imgsz, 
        batch_size=batch_size, 
...
def evaluate_dataset(model_eval, val_loader, imgsz, data_dict, single_cls, save_dir, is_coco, mask_ratio, overlap, conf_thres=0.001 , iou_thres=0.65 ):
...
def run_quantize(weights, data, imgsz, batch_size, hyp, device, save_dir, supervision_stride, iters, no_eval_origin, no_eval_ptq, mask_ratio, overlap, prefix=colorstr('QAT:')):
    ...
    train_dataloader = create_train_dataloader(train_path, imgsz, batch_size, single_cls, stride, hyp, mask_ratio, overlap)
    val_dataloader   = create_val_dataloader(test_path, imgsz, batch_size, single_cls, stride, None, mask_ratio, overlap)
    ...
    if no_eval_origin:
        ...
        with quantize.disable_quantization(model_eval):
            result_eval_origin = evaluate_dataset(model_eval, val_dataloader, imgsz, data_dict, single_cls, save_dir, is_coco, mask_ratio, overlap )
    ...
    if no_eval_ptq:
        ...
        result_eval_ptq = evaluate_dataset(model_eval, val_dataloader, imgsz, data_dict, single_cls, save_dir, is_coco, mask_ratio, overlap  )
    ...
    def per_epoch(model, epoch, lr):
        ...
        model_eval = deepcopy(model).eval()  
        with torch.no_grad():  
            eval_result = evaluate_dataset(model_eval, val_dataloader, imgsz, data_dict, single_cls, save_dir, is_coco, mask_ratio, overlap  )
...
def run_eval(weights, device, data, imgsz, batch_size, save_dir, conf_thres, iou_thres, prefix=colorstr('QAT TEST:')):
    ...
    result_eval = evaluate_dataset(model_eval, val_dataloader, imgsz, data_dict, single_cls, save_dir, is_coco, mask_ratio, overlap ,conf_thres=conf_thres, iou_thres=iou_thres  )
...
if __name__ == "__main__":
    ...
    qat.add_argument('--mask-ratio', type=int, default=4, help='Downsample the truth masks to saving memory')
    qat.add_argument('--no-overlap', action='store_true', help='Overlap masks train faster at slightly less mAP')
    ...
    if opt.cmd == "quantize":
        ...
        run_quantize(
            ...
            opt.no_eval_origin, opt.no_eval_ptq,opt.mask_ratio, opt.no_overlap
        )
